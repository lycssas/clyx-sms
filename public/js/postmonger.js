(function (root, factory) {
  if (typeof define === "function" && define.amd) {
    define("postmonger", [], () => factory(root));
  ...
  let Postmonger;
  const previous = root.Postmonger;
  const _window = root.addEventListener || root.attachEvent ? root : window;
  let Connection;
  let Events;
  let Session;
  ...
  Connection = Postmonger.Connection = function (options) {
    ...
    let connect = options.connect || _window.parent;
  ...
  Session = Postmonger.Session = function () {
    ...
    const connections = [];
    const incoming = new Events();
    const outgoing = new Events();
    const self = this;
    let connection;
    let i;
    let j;
